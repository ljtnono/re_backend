...
public class BackendMenuService {
    ...
    public void saveMenu(BackendMenuSaveDTO dto, User currentUser) {
        ...
        try {
            ...
            // 生成新菜单的路由
            saveNewMenuRoute(menu);
            // 生成新菜单的权限
            generateNewMenuPermission(menu);
        } catch (Exception e) {
    ...
    private void generateNewMenuPermission(Menu menu) {
        // 自动生成菜单的读写权限
        Permission menuPermission = new Permission();
        menuPermission.setProjectName(CommonConstant.PROJECT_NAME_BACKEND_PAGE);
        menuPermission.setMenuId(menu.getId());
        menuPermission.setId(snowflakeIdWorkerUtil.nextId());
        menuPermission.setExpression(menu.getName() + ":write");
        menuPermission.setCreateTime(LocalDateTime.now(ZoneId.of("Asia/Shanghai")));
        menuPermission.setModifyTime(LocalDateTime.now(ZoneId.of("Asia/Shanghai")));
        menuPermission.setDeleted(CommonConstant.ENTITY_NORMAL);
        menuPermission.setName(menu.getTitle());
        menuPermission.setType((byte)1);

        Permission readPermission = new Permission();
        readPermission.setProjectName(CommonConstant.PROJECT_NAME_BACKEND_PAGE);
        readPermission.setMenuId(menu.getId());
        readPermission.setId(snowflakeIdWorkerUtil.nextId());
        readPermission.setExpression(menu.getName() + ":read");
        readPermission.setCreateTime(LocalDateTime.now(ZoneId.of("Asia/Shanghai")));
        readPermission.setModifyTime(LocalDateTime.now(ZoneId.of("Asia/Shanghai")));
        readPermission.setDeleted(CommonConstant.ENTITY_NORMAL);
        readPermission.setType((byte)1);
        readPermission.setName(menu.getTitle() + "-读");
        readPermission.setParentId(menuPermission.getId());

        Permission writePermission = new Permission();
        writePermission.setProjectName(CommonConstant.PROJECT_NAME_BACKEND_PAGE);
        writePermission.setMenuId(menu.getId());
        writePermission.setId(snowflakeIdWorkerUtil.nextId());
        writePermission.setExpression(menu.getName() + ":write");
        writePermission.setCreateTime(LocalDateTime.now(ZoneId.of("Asia/Shanghai")));
        writePermission.setModifyTime(LocalDateTime.now(ZoneId.of("Asia/Shanghai")));
        writePermission.setDeleted(CommonConstant.ENTITY_NORMAL);
        writePermission.setName(menu.getTitle() + "-写");
        writePermission.setType((byte)1);
        writePermission.setParentId(menuPermission.getId());
    }
    ...
    private void checkBackendMenuSaveDTO(BackendMenuSaveDTO dto) {
        ...
        if (!Long.valueOf(-1L).equals(parentId)) {
            ...
            if (!Long.valueOf(-1L).equals(menu.getParentId())) {
                // 父级菜单id不能为子级菜单
